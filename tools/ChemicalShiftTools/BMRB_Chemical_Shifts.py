...
class bmrb_dashboard():
    ...
    def __init__(self):
        ...
        self.data_disordered_corrected = data_disordered_corrected.with_columns(pl.lit('Disordered residues (corrected)').alias('Dataset'))
        self.data_disordered_corrected = self.data_disordered_corrected.with_columns((pl.col('chemical shifts (ppm)')+pl.col('correction addition')).alias('chemical shifts (ppm)'))
    ...
    def cast_datatypes(self, dataframe):
        """
        Cast the datatypes of each column in the dataframe to achieve consistency
        between different data frames (e.g. disordered and disordered corrected dataframes)
        """
        dataframe = dataframe.cast({'temperature (K)': pl.String})
        dataframe = dataframe.cast({'pH': pl.String})
        dataframe = dataframe.cast({'pressure (atm)': pl.String})
        dataframe = dataframe.cast({'ionic strength (M)': pl.String})
...
TYR 175.49651  57.82427  38.76184 121.43652   8.05749   4.51123   2.91782'''
    ...
    def create_dashboard(self):
        ...
        if(self.overlays == 'Individual amino acid/atom'):
            ...
            self.atom = st.sidebar.selectbox("Select Atom:", self.atom_names)
        elif(self.overlays == 'Individual amino acid/atom 2D'):
            self.atom_2D = st.sidebar.selectbox("Select Atom 2D:", ['H-N', 'HA-CA', 'C-N (1-bond)', 'C-N (2-bond)', 'CA-N', 'HB-CB', 'CA-CB', 'H-HA','HA-N', 'HG-CG', 'HD-CD','HA-C', 'H-C', 'H-C (HA-CA + HB-CB + HG-CG + HD-CD)'])
            if(self.atom_2D=='C-N'):
                self.residue = st.sidebar.selectbox("Select Amino Acid (for N atom):", self.residues, help='All residues for the i-1 C carbonyl atom are shown unless filtered manually.')
            else:
                self.residue = st.sidebar.selectbox("Select Amino Acid:", self.residues)
            ...
            self.atom_2D = st.sidebar.selectbox("Select Atom 2D:", ['H-N', 'HA-CA', 'C-N', 'CA-N', 'HB-CB', 'CA-CB', 'HA-N', 'H-HA', 'HG-CG', 'HD-CD', 'HA-C', 'H-C', 'H-C (HA-CA + HB-CB + HG-CG + HD-CD)'], help='Each residue type is defined by the residue of the nitrogen (N) atom. For each residue type, all possible carbonyl carbon (C) i-1 residues are included unless manually filtered.')
        ...
        help_disordered_corrected = """These chemical shifts are from solution state measurements and have been pH and temperature adjusted to pH=7.0, temperature=298K using pH and temperature corrections in POTENCI (Frans Mulder lab - https://doi.org/10.1007/s10858-018-0166-5). Note that this dataset contains only chemical shifts obtained in the following conditions: pH=1-14, temperature=273-330K, ionic strength=0-1M, pressure= 1 atm. N and C terminal residues are also omitted."""
        self.show_disordered_corrected = st.sidebar.checkbox("Disordered residues (pH/temperature corrected)", value=False, help=help_disordered_corrected)
        ...
        if(self.overlays == 'Individual amino acid/atom 3D' or self.overlays == 'All amino acids for selected atom 3D'):
            ...
            elif(self.atom_2D == 'H-HA' or self.atom_2D == 'HA-HB'):
                self.bin_width = st.sidebar.text_input('Histogram x bin width (ppm)', value="0.1")
                self.bin_width2 = st.sidebar.text_input('Histogram y bin width (ppm)', value="0.1")
            else:
    ...
    def plot_amino_acid(self, dataframe):
        ...
        coords = []
        for coord in coordinate_dictionary.values():
            if(type(coord[0])==int):
                coords.append(coord)
            else:
                for c in coord:
        ...
        def find_unit_distance_pairs(points, tolerance=1e-9):
            """
            returning all pairs of atoms separated by a distance of 1
            """
            pairs = []
            for p1, p2 in combinations(points, 2):
                if p1[0] == p2[0] and p1[0] != 0:
                    if(p1[0]!=-1 or p1[1]!=-1):
                        continue
                if(p1[1] == -1 and p2[1] == -1):
                    continue
                if abs(distance(p1, p2) - 1) <= tolerance:
                    pairs.append((p1, p2))
            return pairs

        # Plot bonds between atoms which are separated by a length of 1
        bonds_to_plot = find_unit_distance_pairs(coords)
        ...
        # If self.atom_2D is C-N (1-bond) then the C(i-1)-CA(i-1) bond needs to be plotted as the wildcard
        try:
            if(self.atom_2D == 'C-N (1-bond)'):
                fig_aa.add_trace(go.Scatter(x=[-3,-2], y=[0, 0], mode='lines', line_color='gray', text='', showlegend=False, hoverinfo="skip"), row=1, col=1)
        except:
            pass
        ...
        for atom, coords in coordinate_dictionary.items():
            if(atom in atoms):
                try:
                    if(atom=='C' and self.atom_2D == 'C-N (1-bond)'):
                        color = 'lightgray'
                    else:
                        color = "#F5979A"
                except:
        ...
        fig_aa.update_layout(title={ 'text' : 'General Amino Acid<br>Atom Naming<br>Convention', 'x':0.5, 'xanchor': 'center' })
    ...
    def plot3D(self):
        ...
        if(self.show_disordered and not self.show_all and not self.show_structured and not self.show_disordered_corrected):
            structure = 'disordered'
        elif(self.show_disordered_corrected and not self.show_disordered and not self.show_disordered and not self.show_structured):
            structure = 'disordered (corrected)'
        elif(self.show_all and not self.show_disordered and not self.show_structured and not self.show_disordered_corrected):
            structure = 'all'
        elif(self.show_structured and not self.show_disordered and not self.show_all and not self.show_disordered_corrected):
    ...
    def plot2D(self):
        ...
        if(self.show_disordered and not self.show_all and not self.show_structured and not self.show_disordered_corrected):
            structure = 'disordered'
        elif(self.show_disordered_corrected and not self.show_disordered and not self.show_structured and not self.show_all):
            structure = 'disordered (corrected)'
        elif(self.show_all and not self.show_disordered and not self.show_structured and not self.show_disordered_corrected):
            structure = 'all'
        elif(self.show_structured and not self.show_disordered and not self.show_all and not self.show_disordered_corrected):
        ...
        if(self.overlays == 'Individual amino acid/atom 2D'):
            ...
            if(self.atom_2D != 'H-C (HA-CA + HB-CB + HG-CG + HD-CD)'):
                atom1, atom2 = self.atom_2D.split(' ')[0].split('-')
                if(self.atom_2D == 'C-N (1-bond)'):
                    if(self.filter_by_preceding == True and self.overlay_preceding == False):
                        dataframe1 = self.get_filtered_data(structure = structure, atom = atom1, residue = self.preceding_residue)
                    else:
                        dataframe1 = self.get_filtered_data(structure = structure, atom = atom1, residue = '')
                    dataframe2 = self.get_filtered_data(structure = structure, atom = atom2, residue = self.residue)
                else:
                ...
                for k, pair in enumerate(['HA-CA', 'HB-CB', 'HG-CG', 'HD-CD']):
                    atom1, atom2 = pair.split(' ')[0].split('-')
                    dataframe1 = self.get_filtered_data(structure = structure, atom = atom1, residue = self.residue)
                    dataframe2 = self.get_filtered_data(structure = structure, atom = atom2, residue = self.residue)
                    dataframe = pl.concat([dataframe1, dataframe2], how='vertical')
                    if(k==0):
    ...
    def plot_overlay(self):
        ...
        if(self.show_disordered_corrected):
            residues_to_plot.append('Disordered residues (corrected)')
        if(self.show_all):
    ...
    def filter_atoms_2d(self, dataframe, atom1, atom2):
        ...
        dataframe = dataframe.select(["BMRB entry ID", "residue", "residue number", "atom", "entity id", "temperature (K)", 'physical state', 'pH', 'pressure (atm)', 'ionic strength (M)', 'chemical shifts (ppm)'])
        dataframe = (
                dataframe.group_by(["BMRB entry ID",  "residue number", "entity id", "temperature (K)", 'physical state', 'pH', 'pressure (atm)', 'ionic strength (M)'])
                .agg([
    ...
    def plot_2D_histogram(self, fig: go.Figure, dataframe, atom_pairs: List, values: List, filter_type: str, plot_scatter: bool) -> go.Figure:
        ...
        for i, value in enumerate(values):
            for k, pair in enumerate(atom_pairs):
                atom1, atom2 = pair.split(' ')[0].split('-')
                if(pair == 'C-N (1-bond)'):
                    df1 = dataframe.filter(pl.col('atom')==atom1)
                    df2 = dataframe.filter(pl.col('atom')==atom2)
                    if(filter_type == 'preceding residue type'):
                        df1 = df1.filter(pl.col('residue')==value)
                    elif(filter_type == 'following residue type'):
                        df2 = df2.filter(pl.col('following residue type')==value)
                else:
                ...
                if(self.atom_2D == 'C-N (1-bond)'):
                    # Need to correlate the C from the i-1 residue with the N of residue i
                    try:
                        if(self.overlay_preceding != True):
                            df1 = df1.filter(pl.col('following residue type')==value)
                            df1 = df1.with_columns((pl.col('residue number').cast(pl.Int32)+1).cast(pl.Utf8).alias('residue number'))
                        else:
                            df1 = df1.filter(pl.col('following residue type')==self.residue)
                            df1 = df1.with_columns((pl.col('residue number').cast(pl.Int32)+1).cast(pl.Utf8).alias('residue number'))
                    except:
                        df1 = df1.filter(pl.col('following residue type')==self.residue)
                        df1 = df1.with_columns((pl.col('residue number').cast(pl.Int32)+1).cast(pl.Utf8).alias('residue number'))
            ...
            if (self.overlay_fit):
                ...
                if(n1==1):
                    ...
                    if(self.overlay_potenci):
                        counts, bins = np.histogram(x, bins=int((max(x)-min(x))/float(self.bin_width)), density=True)
                        fig.add_trace(go.Scatter(
                        x=[self.potenci_shifts[self.residue][self.atom], self.potenci_shifts[self.residue][self.atom]],
                        y=[0, max(counts)],
                        mode="lines",
            ...
            for i, atom in enumerate(dataframe["atom"].unique().to_numpy()):
                for dataset_val in residues_to_plot:
                    ...
                    if(dataset_val=='Disordered residues'):
                        opacity = 0.9
                    elif(dataset_val == 'Disordered residues (corrected)'):
                        opacity = 0.6
